...
verify_state() {
  ...
  [ "$plist_name" = "CLAUDE_CONFIG_DIR" ] || fail "plist ProgramArguments[2] 불일치: $plist_name"
  ...
  actual=$(launchctl getenv CLAUDE_CONFIG_DIR 2>/dev/null || true)
  [ "$actual" = "$TARGET_DIR" ] || fail "CLAUDE_CONFIG_DIR 값 불일치: ${actual:-<unset>}"
  echo "--- CLAUDE_CONFIG_DIR 값 OK: $actual"
}
